...
function pauseVid(id) {
	...
	jsOpenServicesMenu.on('click', function(e){
		e.preventDefault();
		jsServicesModal.show();
    document.body.style.overflow = 'hidden';
    if (smoothScroll) smoothScroll.off();
		setTimeout(function(){
			dataActiveOn(jsServicesModal);
		}, 20);
	});
	
	jsOpenProductsMenu.on('click', function(e){
		e.preventDefault();
		jsProductsModal.show();
    document.body.style.overflow = 'hidden';
    if (smoothScroll) smoothScroll.off();
		setTimeout(function(){
			dataActiveOn(jsProductsModal);
		}, 20);
	});
	...
	jsCloseContact.on('click', function(e){
		...
		dataActiveOff(jsServicesModal);
		dataActiveOff(jsProductsModal);
    document.body.style.overflow = 'auto';
		setTimeout(function(){
			jsContactModal.hide();
			jsServicesModal.hide();
			jsProductsModal.hide();
